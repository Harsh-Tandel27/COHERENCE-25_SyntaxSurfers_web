...
import { AlertTriangle, Droplets, Search, Waves, CloudRain, Clock } from "lucide-react"
...
export default function WaterLevelPage() {
  ...
  const processWaterData = (apiData: any) => {
    try {
      ...
      const dischargeData = timeSeries.find((series: any) => series.variable.variableCode[0].value === "00060")

      const gageHeightData = timeSeries.find((series: any) => series.variable.variableCode[0].value === "00065")

      // Extract values
      const currentDischarge = dischargeData?.values[0]?.value[0]?.value || "N/A"
      const currentGageHeight = gageHeightData?.values[0]?.value[0]?.value || "N/A"
      ...
      const historicalData =
        gageHeightData?.values[0]?.value
          ...
          .map((item: any, index: number) => ({
            ...
            discharge: Number.parseFloat(dischargeData?.values[0]?.value[index]?.value || 0),
          }))
          .reverse() || []
    ...
    <main className="container py-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Real-time water level data and flood risk analysis</p>
        </div>
            ...
            <Card>
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <>
                    <div className="text-3xl font-bold">{waterData?.currentLevel.toFixed(1)} ft</div>
                    <p className="text-sm text-muted-foreground">Normal range: 6.5 - 12.0 ft</p>
                    <div className="mt-2 h-2 w-full rounded-full bg-muted">
                      <div
                        ...
                        style={{ width: `${Math.min((waterData?.currentLevel / 20) * 100, 100)}%` }}
                      />
                    </div>
                  </>
                )}
            ...
            <Card>
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <>
                    <div className="text-3xl font-bold">{waterData?.reservoirCapacity.toFixed(1)}%</div>
            ...
            <Card>
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <>
                    <div className="text-3xl font-bold">{waterData?.floodRisk.level}</div>
                    <p className="text-sm text-muted-foreground">
                ...
                <div className="h-[300px] flex items-center justify-center">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      >
                        {waterData?.reservoirData.map((_: any, index: number) => (
                          <Cell key={`cell-${index}`} fill={index === 0 ? "#0088FE" : "#EEEEEE"} />
                        ))}
                      </Pie>
                      <Tooltip />
                    </PieChart>
        ...
        <TabsContent value="historical" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Last 24 hours of water level data</CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                <Skeleton className="h-[300px] w-full" />
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={waterData?.historicalData}>
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={waterData?.historicalData}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="time" />
                      <YAxis />
                      <Tooltip />
                ...
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="value" name="Value" fill="hsl(var(--chart-3))" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              )}
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">Water quality is suitable for all uses</p>
                    <div className="mt-2 h-2 w-full rounded-full bg-muted">
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <>
                    ...
                    <p className="text-sm text-muted-foreground">Below threshold for all measured pollutants</p>
                    <div className="mt-2 h-2 w-full rounded-full bg-muted">
                  ...
                  <>
                    <div className="text-3xl font-bold">
                      {waterData?.precipitationForecast
                        .reduce((sum: number, day: any) => sum + day.precipitation, 0)
                        .toFixed(1)}{" "}
                      mm
                    </div>
                    <p className="text-sm text-muted-foreground">Total expected over next 7 days</p>
                  </>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <>
                    <div className="text-3xl font-bold">
                      {waterData?.precipitationForecast.find((day: any) => day.precipitation > 10)?.day || "None"}
                    </div>
                    <p className="text-sm text-muted-foreground">
                      {waterData?.precipitationForecast.find((day: any) => day.precipitation > 10)
